...
class _AttendanceReportPageState extends ConsumerState<AttendanceReportPage> {
  ...
  Widget _buildExcelPreviewTable() {
    ...
    final totalItems = _filteredExcelRows.length;
    ...
    final paginatedExcelRows = _filteredExcelRows.sublist(startIndex, endIndex);
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                return SingleChildScrollView(
                  ...
                  rows: paginatedExcelRows.map((row) {
                    return DataRow(
                      cells: [
                        DataCell(
                          Text(
  ...
  Widget _buildScansPreviewList() {
    if (_filteredScans.isEmpty) {
      return _buildEmptyState();
    }

    final totalItems = _filteredScans.length;
    final totalPages = (totalItems / _rowsPerPage).ceil();
    final safePage = _currentPage >= totalPages ? (totalPages > 0 ? totalPages - 1 : 0) : _currentPage;

    final startIndex = safePage * _rowsPerPage;
    final endIndex = startIndex + _rowsPerPage > totalItems ? totalItems : startIndex + _rowsPerPage;
    final paginatedScans = _filteredScans.sublist(startIndex, endIndex);
